...
export const update = ({ d3_refs, newValue, config }) => {
  const actualArray = [0,40,65,87,100]
  const visualArray = [0,3,10,20,100]
  
  let mathematicalValue
  //  = 107.1991 + (0.1815371 - 107.1991)/(1 + (newValue/5.450825)^0.9565319)
  if(0<=newValue){
     
     mathematicalValue = 13.33333*newValue + 0
     
    }
  if(3<=newValue) {
 
    mathematicalValue = 3.571429*newValue + 29.28571
    

  }
  if(10<=newValue) {
   
    mathematicalValue = 2.2*newValue + 43
  

  }
  if(20<=newValue) {
   
    mathematicalValue =0.1625*newValue + 83.75
  }
  
  const scale = configureScale(config)
  
  const ratio = scale(mathematicalValue)
